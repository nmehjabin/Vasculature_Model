...
# class Vasculature:
#     def __init__(self, grid, initial_positions):
#         self.grid = grid
...
#         #To keep track of the tip cell growth direction
#         self.directions = {pos: None for pos in initial_positions} 
...
#         ]
#         #neighbor prints location 
#         # Filter neighbors within grid bounds that are empty
#         empty_neighbors = []  # Initialize an empty list to store valid neighbors
#         #we go through each location by comparing the value stored in the grid with the location
#         for n in neighbors:
#             # Check if the neighbor is within bounds and empty
#             if 0 <= n[0] < self.grid.shape[0] and 0 <= n[1] < self.grid.shape[1] and self.grid[n] == 0:
#                 if prev_direction is None or (n[0]-x,n[1]-y) != (-prev_direction[0],-prev_direction[1]):
# #prev_direction has dx, dy: (n[0]-x, n[1]-y) is the direction vector from the current cell (position) to a potential neighbor (n)
#                     empty_neighbors.append(n)  # Add valid neighbor to the list
...
#             # self.grid[tip] = 1  # Ensure the current tip position is updated before moving

#             #Get the dirrection of the current tip and find valid neighbors
#             prev_direction = self.directions.get(tip, None)

#             empty_neighbors = self.get_empty_neighbors(tip,prev_direction)
#             if empty_neighbors:
#                 # Randomly choose one neighbor for normal growth
#                 new_position = random.choice(empty_neighbors)
#                 self.grid[tip] = 1  # Mark current tip as part of the vasculature
...
#             else:
#                 self.active_tips[i] = False  # Deactivate if no neighbors
...
#     #         print(f"Tip position: {tip}, Grid value: {self.grid[tip]}")

#     #         # Check if tip collides with pre-existing vasculature
#     #         if self.grid[tip] == 1:
#     #             print(f"Anastomosis occurred at {tip} with pre-vasculature.")
#     #             to_deactivate.append(i)
#     #             continue

#     #         # Check proximity to other tips (i.e., if tips are close to each other)
#     #         for j, other_tip in enumerate(self.tip_cells):
#     #             if i != j and self.active_tips[j] and self.is_neighbor(tip, other_tip):
#     #                 print(f"Anastomosis occurred between tips at {tip} and {other_tip}.")
#     #                 to_deactivate.extend([i, j])
#     #                 break

#     #     # Deactivate tips involved in anastomosis
#     #     for idx in set(to_deactivate):
#     #         self.active_tips[idx] = False

    


#     def is_neighbor(self, cell1, cell2):
#         """Check if two cells are within 1 cell of each other (Moore neighborhood)."""
#         x1, y1 = cell1
#         x2, y2 = cell2
...
class Vasculature:
    def __init__(self, grid, initial_positions, vegf_field):
        self.grid = grid
        self.tip_cells = initial_positions  # List of active tip positions
        self.active_tips = [True] * len(initial_positions)  # Track which tips are active
...
# where the VEGF is highest and return the sorted neighbors
    def choose_growth_direction(self, empty_neighbors):
        ...
        #     return None
        # # Sort neighbors by VEGF concentration (highest first)
        # sorted_neighbors = sorted(empty_neighbors, key=lambda n: self.vegf_field[n], reverse=True)
        # # print("Sorted Neigbors##", sorted_neighbors)
        # # Prefer the neighbor with the highest VEGF concentration
...
# as well as directions 
    ...
    def check_anastomosis(self):
        ...
        for i, tip in enumerate(self.tip_cells):
            ...
            # Check if the tip is near another tip
            ...
            for other_tip in self.tip_cells:
                if tip != other_tip and self.is_near(tip, other_tip):
                    ...
                    # Handle anastomosis (e.g., deactivate the tip or merge paths)

            # # Check if the tip collides with pre-existing vasculature
            # if self.grid[tip] == 1:
            #     print(f"Tip at {tip} has collided with existing vasculature")
            #     # Handle collision (e.g., deactivate the tip)
